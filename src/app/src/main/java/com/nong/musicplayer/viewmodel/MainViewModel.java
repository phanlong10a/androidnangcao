...
public class MainViewModel extends ViewModel {
    ...
    public MutableLiveData<List<OnlineSong>> onlineSongList;
    ...
    public MutableLiveData<List<OnlineSong>> getOnlineSongList() {
        if (onlineSongList != null) return onlineSongList;
        return onlineSongList = new MutableLiveData<>();
    }


    public void setOnlineSongList(List<OnlineSong> musicList) {
        List<String> excludedFolderList = MPPreferences.getExcludedFolders(App.getContext());
        List<OnlineSong> songs = new ArrayList<>();

        for (OnlineSong music : musicList) {
            if (!excludedFolderList.contains(music.relativePath))
                songs.add(music);
        }
        if (onlineSongList == null) {
            onlineSongList = new MutableLiveData<>();
        }
        onlineSongList.setValue(songs);
    }
